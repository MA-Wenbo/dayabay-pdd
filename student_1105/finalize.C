...
//        printf("time: %f\n", time);
        ...
        Muon->GetEntry(mnt);
        float tm = (float) msec+ (float)mnanosec*1e-9;
        cout << msec-t0 << ' ' <<  mnanosec << endl;
        while((tm-t0)<time_delay-600e-6 && mnt<total_muons){
            ++mnt;
            Muon->GetEntry(mnt);
            tm = (float) msec+ (float)mnanosec*1e-9;
        }
//        printf("tm is at %f, td is at %f.\n", tm-t0, time_delay);
        if(tm-t0<600e-6+time_delay && tm-t0>time_delay-600e-6){
            mnt = max(mnt-100,0);
            printf("ABORT BY MUON at event %d with time %f and time %f\n", i, tm-t0, time_delay);
            continue;
        }
        mnt = max(mnt-100,0);
        ...
        while(ta-t0<time_delay-1000e-6 && ant<total_ads){
            ++ant;
            Ad->GetEntry(ant);
        }
//        printf("ta is at %f, td is at %f.\n", ta-t0, time_delay);
        if(ta-t0<=1000e-6+time_delay && ta-t0>=time_delay-1000e-6){
            ant = max(ant-100,0);
//            printf("ABORT BY AD at event %d\n", i);
            continue;
        }
        ant = max(ant-100,0);
